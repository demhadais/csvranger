...
/// A CSV value found in the `metrics_summary.csv` files produced by 10x Genomics *ranger pipelines.
///
/// If parsing a whole CSV at once, it's useful to have one `enum` that can fit any of the values in the file. If you only need a specific field in the file with a known numeric type, you may want to deserialize it directly or use [`parse_legacy_csv_value_as_f64`] and/or [`parse_legacy_csv_value_as_i64`] if dealing with outputs generated by older pipelines.
...
impl TenxCsvValue {
    ...
    /// - `spaceranger count >= 4`
    ///
    /// then this is the correct method. Note that [`TenxCsvValue::from_legacy_csv_value`] will still work for modern pipelines, but because it uses a regular expression for parsing, it will be less performant. Note also that other versions of `spaceranger` and `cellranger-atac` may produce outputs compatible with this method, but they are currently untested.
